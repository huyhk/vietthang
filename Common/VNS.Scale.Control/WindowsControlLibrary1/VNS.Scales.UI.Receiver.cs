...
namespace VNS.Scales.UI
{
    ...
    public partial class Receiver : UserControl
    {
        ...
        void helper_ErrorReceived(object sender, string data)
        {
            if ((sender as ScaleHelper).Status != HelperStatus.OPEN)
            {
                if (MessageBox.Show(string.Format("Error opening port [{0}], retry?", data), "Error", MessageBoxButtons.RetryCancel, MessageBoxIcon.Error) == DialogResult.Retry)
                    helper.Open();
            }
        }
        /// <summary>
        ...
        void helper_DataReceived(object sender, string data)
        {
            ...
            {
                System.Text.RegularExpressions.Regex re = new System.Text.RegularExpressions.Regex(this.helper.Config.DataPatern);
                //Write2Log.WriteLogs("Receiver", "DataPatern", this.helper.Config.DataPatern);
                //Write2Log.WriteLogs("Receiver", "BaudRate", helper.Config.BaudRate.ToString());
                //Write2Log.WriteLogs("Receiver", "DataReceived", data);
                if (re.IsMatch(data))
                {
                    //Write2Log.WriteLogs("Receiver", "DataReceived Match 0", re.Match(data).Groups[0].Value);
                    //Write2Log.WriteLogs("Receiver", "DataReceivedMatch 1", re.Match(data).Groups[1].Value);
